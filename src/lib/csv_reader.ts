...
import { ReportRecord, ImportError, CSVPreview } from '../types.js';
...
export function parseAndValidateCSV(csvText: string): { errors: ImportError[]; preview?: CSVPreview; records?: ReportRecord[] } {
  ...
  const pad = (num: number) => num.toString().padStart(2, '0');
  const dateStart = `${pad(minDate.getUTCDate())}/${pad(minDate.getUTCMonth() + 1)}/${minDate.getUTCFullYear()}`;
  const dateEnd = `${pad(maxDate.getUTCDate())}/${pad(maxDate.getUTCMonth() + 1)}/${maxDate.getUTCFullYear()}`;
