import { ReportRecord, ExecutiveMetrics, ChartDataPoints } from '../src/types.js';
...
export function performCalculations(records: ReportRecord[]): { metrics: ExecutiveMetrics; charts: ChartDataPoints } {
  ...
  Object.keys(teamStats).forEach(t => {
    if (teamStats[t] > equipeMaisProdutivaQtd) {
  ...
  const metrics: ExecutiveMetrics = {
    ...
    responsavelTopQtd,
    resolvidosOS,
    taxaResolucaoOS: Math.round(taxaResolucaoOS * 10) / 10,
    modeloCritico,
  ...
  const indicadoresAvancados = [
    { indicador: 'Taxa Geral de Resolução (Caixa OS)', valor: `${Math.round(taxaResolucaoOS * 10) / 10}%`, status: taxaResolucaoOS > 60 ? 'OK' : taxaResolucaoOS > 40 ? 'ATENÇÃO' : 'CRÍTICO' as any },
